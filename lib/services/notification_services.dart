...
class NotifyHelper {
  ...
  scheduledNotification(int hour, int minutes, Task task) async {
    await flutterLocalNotificationsPlugin.zonedSchedule(
      ...
      payload: '${task.title}|${task.note}|${task.endTime}|',
    );
  }
  ...
  tz.TZDateTime _nextInstanceOfTenAM(int hour, int minutes, int remind, String repeat, String date) {
    ...
    scheduledDate = afterRemind(scheduledDate, remind);
    ...
    tz.TZDateTime afterRemind(tz.TZDateTime scheduledDate, int remind) {
      if (remind == 5) {
        scheduledDate = scheduledDate.subtract(const Duration(minutes: 5));
      }else{
        scheduledDate = scheduledDate.subtract(const Duration(seconds: 3));
      }
      if (remind == 10) {
